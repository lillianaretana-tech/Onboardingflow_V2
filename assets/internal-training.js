...
async function manage(training){const[inviteResult,videoResult]=await Promise.all([client().from('of_training_invitees').select('id,employee_id,status,notes,checkin_at,employee:of_employees(employee_number,full_name,project_name)').eq('training_id',training.id),training.modality==='video'?client().from('of_training_videos').select('*').eq('training_id',training.id).order('video_order'):Promise.resolve({data:[]})]);if(inviteResult.error)return alert(inviteResult.error.message);const invited=inviteResult.data||[],videos=videoResult.data||[],selected=new Set(invited.map(x=>x.employee_id)),projects=[...new Set(employees.map(x=>x.project_name))].sort();
  ...
  if(training.modality==='video'){
    d.querySelector('[data-add-video]').onclick=async()=>{const btn=d.querySelector('[data-add-video]'),title=d.querySelector('[data-new-video-title]').value.trim(),url=d.querySelector('[data-new-video-url]').value.trim();if(!title||!url)return alert('Complete título y enlace del video.');btn.disabled=true;btn.textContent='Guardando...';const{error}=await client().from('of_training_videos').insert({training_id:training.id,title,video_url:url,video_order:videos.length+1});btn.disabled=false;btn.textContent='Guardar video';if(error)return alert(error.message);alert('Video guardado y agregado a la capacitación.');d.remove();manage(training)};
